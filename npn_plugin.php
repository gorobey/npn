 ...
 * Plugin URI:  http://gorobey.it
 * Description: Simply notifies users if a feture post has been published. This is an addon for User-Access-Manager. Users will only be notified if they have access. Your subscribers can also decide which posts they would like to be noticed on.
 * Version:     1.0.11
 * Author:      Jan Eichhorn mod by Giulio Gorobey
 * Author URI:  http://gorobey.it
 * License:     GPLv2
 */
...
add_action ( 'future_to_publish', 'npn_notify' );
...
function npn_get_allowed_groups($postID){
	global $wpdb;
	$allowed_group_ids = $wpdb->get_results( "SELECT group_id FROM ".$wpdb->prefix."uam_accessgroup_to_object WHERE object_id = ".$postID->ID." AND object_type = 'post' ");
